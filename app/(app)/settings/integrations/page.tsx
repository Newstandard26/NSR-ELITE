...
export default function IntegrationsSettingsPage() {
  ...
  // --- Property data diagnostic: what the running server resolves ---
  const provider = process.env.PROPERTY_DATA_PROVIDER ?? null;
  const attomKey = mask(process.env.ATTOM_API_KEY);
  const batchKey = mask(process.env.BATCHDATA_API_KEY);
  ...
  const activeProvider =
    provider === "batchdata" && batchKey.set ? "batchdata" : provider === "attom" && attomKey.set ? "attom" : "mock";
  const providerKey = provider === "batchdata" ? batchKey : attomKey;

  let reason = "";
  if (activeProvider === "mock") {
    if (!provider) reason = "PROPERTY_DATA_PROVIDER is not set on this deployment (set it, then redeploy).";
    else if (provider !== "attom" && provider !== "batchdata")
      reason = `PROPERTY_DATA_PROVIDER is "${provider}" — it must be exactly "batchdata" or "attom".`;
    else if (!providerKey.set) reason = `The API key for "${provider}" is not set on this deployment.`;
  }

  const label = activeProvider === "batchdata" ? "BatchData (live data)" : activeProvider === "attom" ? "ATTOM (live data)" : "MOCK (sample data)";

  const propRows = [
    { label: "Provider", value: provider ?? "— not set —", ok: provider === "attom" || provider === "batchdata", hint: "PROPERTY_DATA_PROVIDER (batchdata | attom)" },
    { label: "BatchData API Key", value: batchKey.masked, ok: batchKey.set, hint: "BATCHDATA_API_KEY" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <h2 className="text-lg font-semibold">Property Data</h2>
        <div
          className={`rounded-2xl border p-3 ${
            activeProvider !== "mock" ? "border-green-500/40 bg-green-500/5" : "border-amber-500/40 bg-amber-500/5"
          }`}
        >
          <p className="text-sm font-semibold">
            Active provider:{" "}
            <span className={activeProvider !== "mock" ? "text-green-400" : "text-amber-400"}>{label}</span>
          </p>
